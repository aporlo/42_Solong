		...
		while (y < data->map.width)
		{
			if (data->map.grid[x][y])
			{
				if (data->map.grid[x][y] == '1')
				{
					...
					new_data = ft_lstnew(content);
					ft_lstadd_back(&data->wall, new_dat);
				}
			}
			y++;
		}
		...
		while (y < data->map.width)
		{
			if (data->map.grid[x][y])
			{
				if (data->map.grid[x][y] == 'P')
				{
					...
					p->v.x = x;
					p->v.y = y;
					p->img.ptr = mlx_xpm_file_to_image(data->mlx,
							p->addr, &image.w, &image.h);
				}
			}
			y++;
		}
		...
		while (y < data->map.width)
		{
			if (data->map.grid[x][y])
			{
				if (data->map.grid[x][y] == 'E')
				{
					...
					e->v.x = x;
					e->v.y = y;
					e->img.ptr = mlx_xpm_file_to_image(data->mlx,
							e->addr, &image.w, &image.h);
				}
			}
			y++;
		}
